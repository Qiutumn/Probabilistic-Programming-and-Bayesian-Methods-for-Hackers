...
#+END_SRC
[[file:Ch3_IntroMCMC_zh_files/img_003_0.png]]
...
plt.title("$Exp(3), Exp(10)$ 先验地貌;\n另一个视角");
#+END_SRC
[[file:Ch3_IntroMCMC_zh_files/img_005_0.png]]
...
plt.ylim(0, 5);
#+END_SRC
[[file:Ch3_IntroMCMC_zh_files/img_008_0.png]]
...
我们应该在先验曲面和观测数据共同生成的这片扭曲后验空间里,探索出后验"山峰"所在的位置。然而,我们不能天真地对这个空间做穷举搜索:任何一位计算机科学家都会告诉你,遍历 $N$ 维空间的难度是随 $N$ 指数级增长的——空间的规模会随着 $N$ 的增大而迅速爆炸(参见[[http://en.wikipedia.org/wiki/Curse/of/dimensionality][维度灾难]])。那我们还有什么希望找到这些隐藏的山峰呢?MCMC 背后的思路,是对这片空间进行一次"聪明"的搜索。说"搜索"其实暗示着我们在寻找某个特定的点,但这可能不太准确,因为我们真正要找的其实是一整片广阔的山峰区域。
...
除了 MCMC,还有其他方法可以用来求出后验分布。拉普拉斯近似(Laplace approximation)用简单的函数来近似后验分布。一种更高级的方法是[[http://en.wikipedia.org/wiki/Variational/Bayesian/methods][变分贝叶斯]](Variational Bayes)。拉普拉斯近似、变分贝叶斯和经典 MCMC 这三种方法各有优劣,本书只专注于 MCMC。话说回来,我的朋友 Imri Sofar 喜欢把 MCMC 算法分成"很烂"和"特别烂"两类,他把 PyMC 使用的这种 MCMC 归为仅仅是*烂* ;)
...
print(data[:10], "...")
#+END_SRC
[[file:Ch3_IntroMCMC_zh_files/img_012_0.png]]
...
新版 PyMC 还有另一件有意思的事。PyMC 现在采用了一种强大的新采样原理,叫做哈密顿蒙特卡洛(Hamiltonian Monte Carlo, HMC)。这里我们不会深入展开,因为它涉及比较复杂的物理原理。但我们至少应该知道,[[https://www.pymc.io/projects/docs/en/stable/learn/core/notebooks/pymc/overview.html#pymc-overview][HMC 和 NUTS 利用了似然函数的梯度信息,相比传统采样方法能实现快得多的收敛速度,尤其是在模型规模较大的时候。]]
...
plt.legend();
#+END_SRC
[[file:Ch3_IntroMCMC_zh_files/img_023_0.png]]
...
plt.show()
#+END_SRC
[[file:Ch3_IntroMCMC_zh_files/img_025_0.png]]
[[file:Ch3_IntroMCMC_zh_files/img_025_1.png]]
[[file:Ch3_IntroMCMC_zh_files/img_025_2.png]]
...
plt.xlabel("步数");
#+END_SRC
[[file:Ch3_IntroMCMC_zh_files/img_028_0.png]]
...
plt.plot()
#+END_SRC
[[file:Ch3_IntroMCMC_zh_files/img_029_0.png]]
...
plt.tight_layout()
#+END_SRC
[[file:Ch3_IntroMCMC_zh_files/img_031_0.png]]
...
plt.plot()
#+END_SRC
[[file:Ch3_IntroMCMC_zh_files/img_032_0.png]]
[[file:Ch3_IntroMCMC_zh_files/img_032_1.png]]
...
plt.title("数据点的后验标签");
#+END_SRC
[[file:Ch3_IntroMCMC_zh_files/img_034_0.png]]
...
plt.xlabel("数据点的取值");
#+END_SRC
[[file:Ch3_IntroMCMC_zh_files/img_036_0.png]]
...
#+END_SRC
[[file:Ch3_IntroMCMC_zh_files/img_038_0.png]]
...
#+END_SRC
[[file:Ch3_IntroMCMC_zh_files/img_040_0.png]]
...
\begin{align}
& P(L/x = 1| x = 175 ) \gt P(L/x = 0| x = 175 ) \\\\[5pt]
& \frac{ P( x=175  | L/x = 1  )P( L/x = 1 ) }{P(x = 175) } \gt \frac{ P( x=175  | L/x = 0  )P( L/x = 0 )}{P(x = 175) }
\end{align}
...
=find_MAP()= 函数很灵活,允许用户选择使用哪种优化算法(毕竟这本质上是一个优化问题:我们要找到能让地貌取值最大化的那组参数),因为不同的优化算法效果并不都一样。这个函数调用默认使用的优化算法是 Broyden-Fletcher-Goldfarb-Shanno([[https://en.wikipedia.org/wiki/Broyden-Fletcher-Goldfarb-Shanno_algorithm][BFGS]])算法,用来寻找对数后验的最大值。你也可以选用 =scipy.optimize= 模块里的其他优化算法。例如,你可以调用 =find_MAP(fmin=scipy.optimize.fmin_powell)=,使用 Powell 方法——这是 PyMC 博主 [[http://healthyalgorithms.com/][Abraham Flaxman]] [1] 最喜欢的方法。默认算法通常已经够用了,但如果收敛缓慢或者无法保证收敛,不妨试试 Powell 方法或其他可用的算法。
...
plt.legend();
#+END_SRC
[[file:Ch3_IntroMCMC_zh_files/img_046_0.png]]
...
#+END_SRC
[[file:Ch3_IntroMCMC_zh_files/img_048_0.png]]
...
#+END_SRC
[[file:Ch3_IntroMCMC_zh_files/img_051_0.png]]
...
#+END_SRC
[[file:Ch3_IntroMCMC_zh_files/img_054_0.png]]
[[file:Ch3_IntroMCMC_zh_files/img_054_1.png]]
[[file:Ch3_IntroMCMC_zh_files/img_054_2.png]]
