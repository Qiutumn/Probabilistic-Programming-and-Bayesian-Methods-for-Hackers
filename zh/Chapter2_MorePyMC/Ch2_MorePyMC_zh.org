...
其中 0 和 4 是 =DiscreteUniform= 特有的、随机变量的下界和上界。[[https://docs.pymc.io/en/stable/api.html][PyMC 文档]]里列出了各类随机变量的具体参数(如果你在用 IPython,也可以用 =??= 查看)。
...
=shape= 参数还能解决另一个烦人的情形:假设你想为很多个变量 $\beta_i,\; i=1,\dots,N$ 建模。与其像下面这样为每一个变量起一个随意的名字:
    ...
    beta/1 = pm.Uniform("beta/1", 0, 1)
    beta/2 = pm.Uniform("beta/2", 0, 1)
    ...
...
创建确定性变量的方式和创建随机变量类似。我们只需要调用 PyMC 的 =Deterministic= 类,并传入我们想要的函数:
    ...
    deterministic/variable = pm.Deterministic("deterministic variable", some/function/of/variables)

在各种场合下,我们都可以把对象 =some_deterministic_var= 当成一个变量,而不是一个 Python 函数来使用。

调用 [[https://www.pymc.io/projects/docs/en/latest/api/generated/pymc.Deterministic.html?highlight=Deterministic][=pymc.Deterministic=]] 是最直接的创建确定性变量的方式,但并不是唯一的方式。加法、指数运算等基本操作会隐式地创建确定性变量。例如,下面这行代码会返回一个确定性变量:
...
举例来说,运行下面这段代码(当然,前提是你已经先定义好了模型会话):
    ...
    def subtract(x, y):
        return x - y
    with model:
        stochastic/1 = pm.Uniform("U/1", 0, 1)
        stochastic/2 = pm.Uniform("U/2", 0, 1)

        det/1 = pm.Deterministic("Delta", subtract(stochastic/1, stochastic_2))
...
在本书接下来的部分,我们会多次用到 =pytensor= 的各种函数来辅助构建模型。如果你对 =pytensor= 本身更感兴趣,可以查阅它的[[https://pytensor.readthedocs.io/en/latest/][官方文档]]。
...
plt.xlim(0, 8);
#+END_SRC
[[file:Ch2_MorePyMC_zh_files/img_029_0.png]]
...
下图给出了这个过程的图形化展示,箭头表示"父子"关系(由 [[https://docs.daft-pgm.org/en/latest/][Daft Python 库]] 提供)。
...
plt.legend();
#+END_SRC
[[file:Ch2_MorePyMC_zh_files/img_042_0.png]]
...
#+END_SRC
[[file:Ch2_MorePyMC_zh_files/img_044_0.png]]
...
plt.legend();
#+END_SRC
[[file:Ch2_MorePyMC_zh_files/img_056_0.png]]
...
#+END_SRC
[[file:Ch2_MorePyMC_zh_files/img_059_0.png]]
...
ax.plot()
#+END_SRC
[[file:Ch2_MorePyMC_zh_files/img_061_0.png]]
...
plt.legend(loc="upper right");
#+END_SRC
[[file:Ch2_MorePyMC_zh_files/img_070_0.png]]
...
#+END_SRC
[[file:Ch2_MorePyMC_zh_files/img_075_0.png]]
...
plt.legend();
#+END_SRC
[[file:Ch2_MorePyMC_zh_files/img_094_0.png]]
...
plt.legend();
#+END_SRC
[[file:Ch2_MorePyMC_zh_files/img_102_0.png]]
...
#+END_SRC
[[file:Ch2_MorePyMC_zh_files/img_107_0.png]]
...
plt.legend();
#+END_SRC
[[file:Ch2_MorePyMC_zh_files/img_109_0.png]]
...
#+END_SRC
[[file:Ch2_MorePyMC_zh_files/img_111_0.png]]
...
#+END_SRC
[[file:Ch2_MorePyMC_zh_files/img_114_0.png]]
...
plt.legend();
#+END_SRC
[[file:Ch2_MorePyMC_zh_files/img_120_0.png]]
...
plt.plot()
#+END_SRC
[[file:Ch2_MorePyMC_zh_files/img_121_0.png]]
...
plt.xlabel("温度");
#+END_SRC
[[file:Ch2_MorePyMC_zh_files/img_124_0.png]]
...
#+END_SRC
[[file:Ch2_MorePyMC_zh_files/img_126_0.png]]
...
#+END_SRC
[[file:Ch2_MorePyMC_zh_files/img_129_0.png]]
...
于是我们创建:
    ...
    simulated/data = pm.Bernoulli("simulation/data", p)
...
for i in range(4):
    ...
    plt.scatter(temperature, simulations[1000 * i, :], color="k",
                s=50, alpha=0.6)
#+END_SRC
[[file:Ch2_MorePyMC_zh_files/img_132_0.png]]


注意上面几张图各不相同(如果你能想到更清晰的展示方式,欢迎提交 pull request,也欢迎在[[http://stats.stackexchange.com/questions/53078/how-to-visualize-bayesian-goodness-of-fit-for-logistic-regression][这里]]回答!)。
...
下面这种图形化检验方法,是逻辑回归领域一种新颖的数据可视化手段,叫做*分离图*(separation plot)[8]。对于我们想要比较的一组模型,每个模型都会被画在各自的分离图上。分离图的技术细节我留给非常易读的[[https://onlinelibrary.wiley.com/doi/10.1111/j.1540-5907.2011.00525.x][原始论文]]去介绍,这里只总结一下它的用法。
...
#+END_SRC
[[file:Ch2_MorePyMC_zh_files/img_138_0.png]]
...
plt.title("常数预测模型");
#+END_SRC
[[file:Ch2_MorePyMC_zh_files/img_140_0.png]]
[[file:Ch2_MorePyMC_zh_files/img_140_1.png]]
[[file:Ch2_MorePyMC_zh_files/img_140_2.png]]
[[file:Ch2_MorePyMC_zh_files/img_140_3.png]]
...
plt.ylabel(r"$\beta$");
#+END_SRC
[[file:Ch2_MorePyMC_zh_files/img_144_0.png]]
